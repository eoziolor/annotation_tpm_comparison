...
* Use the below bash script to download the Ensembl genomes across species for v 95.
* Environment: This snippet was ran as an executable bash script in a command line environment. The necessary tools to run this are salmon version 1.4.0. This can be installed by downloading https://github.com/COMBINE-lab/salmon/releases/download/v1.4.0/salmon-1.4.0_linux_x86_64.tar.gz and running ```tar -xvzf salmon-1.4.0_linux_x86_64.tar.gz```
* Files: None required, downloaded files through script
...
for i in {1..7}; do
	...
	my_salmon=/root/program/salmon-latest_linux_x86_64/bin/salmon
...
* Once human sample is selected, I took the quant file and grabbed a random gene from 10 sets of bins, separated by expression levels betweeen 0.5 and 5 log10 transcripts per million (tpm). Here is the script used to select those genes. Those genes are:

"ENSG00000091704","ENSG00000168928","ENSG00000010438","ENSG00000198763","ENSG00000254772","ENSG00000142676","ENSG00000138161","ENSG00000140688","ENSG00000154556","ENSG00000198561"

* Environment: This was run in R environment with the shown packages loaded.
* Files: quantitation file for teh chosen sample (provided).
...
panc<-read.table("/root/tpm_genome_analysis/data/human_sample/quant.sf", header = TRUE, sep = '\t')
...
# }
# 
# write.table(df,"/root/tpm_genome_analysis/data/human_sample/theten.tsv", sep = '\t', col.names = TRUE, row.names = FALSE, quote = FALSE)

# Reading in genes from the initial selection
df<-read.table("/root/tpm_genome_analysis/data/human_sample/theten.tsv", sep = '\t', header = TRUE, stringsAsFactors = FALSE)
...
exportFASTA(tseq, "/root/tpm_genome_analysis/data/human_sample/theten.fa")
```
...
* Environment: The tools necessary for this snippet are meme-5.0.5. This can be installed by downloading the following software (https://meme-suite.org/meme/meme-software/5.0.5/meme-5.0.5.tar.gz) and running 
...
cd meme-5.0.5
./configure --prefix=/hpc/grid/scratch/oziole/program/meme --with-url=http://meme-suite.org/ --enable-build-libxml2 --enable-build-libxslt
make
make test
make install
```
...
* Files: human genome downloaded from chunk 1. The ten chosen genes in fasta format creted chunk 2.

```{bash}
# Defining locations for programs and files
mysub=/root/program/meme/libexec/meme-5.0.5/fasta-subsample
myhomo=/root/data/sequencing/genomes/human/homo_sapiens.fa
subhomo=/root/data/sequencing/genomes/human/nospike_homo_sapiens.fa
myout=/root/size_genome/data/genomes_human/
genes=/root/size_genome/data/genes_human/theten.fa
...
* Taking the selected 10 transcripts and putting them back into the fasta files of subsampled genomes
* Environment: This snippet was run in linux command line with no necessary tools.
* Files: theten.fa created by chunk 2 and subsampled genomes created by chunk 3.

```{bash}
parent=/root/size_genome/data/genomes_human/
genes=/root/size_genome/data/genes_human/theten.fa
...
* Environment: This code was ran on an LSF commandline cluster. The only tool necessary is salmon (installation described above)
* Files: re-implanted genomes from chunk 4.
...
* The name of the selected HPA sample is itox0000633 (equivalent to the sample from HPA mentioned at top of script)
* Environment: The script below was ran as a job on an LSF cluster. The only software necessary is salmon (as described above).
* Files: Fastq files for HPA sample (https://www.ebi.ac.uk/ena/data/view/PRJEB6971) and indexed re-implanted genomes from chunk 4-5

```{bash}
# Defining directories and samples
parent=/root/size_genome/data/genomes_human/
cd $parent
sample=/root/size_genome/data/sample/itox000633
name=itox000633
...
"/root/program/salmon-latest_linux_x86_64/bin/salmon quant \
-i ${parent}/${i} \
-p 4 \
--validateMappings \
-l IU \
...
* Environment: These commands were ran in commandline on an LSF cluster. No software necessary.
* Files: quantitation files produced from chunk above.
...
genes=/root/size_genome/data/genes/theten.fa
...
* Environment: These commands were ran in commandline on an LSF cluster. No software necessary.
* Files: Continued work with quantitation files created from chunks above.

```{bash}
parent=/root/size_genome/data/genomes_human/summary
cd $parent
...
* Here we used tximport to read in the quantitation files
* Environment: These commands were ran in R 3.6.1. The packages loaded via library() function are the only ones used in below chunk. 
* Files: only file necessary is the gtf file for human annotation that can be downloaded via this link: (ftp://ftp.ensembl.org/pub/release-95/gtf/homo_sapiens/Homo_sapiens.GRCh38.95.gtf.gz). It is also downloaded by chunk 1. The other two files noted are created from chunks above.
...
orig<-read.table("/root/tpm_genome_analysis/data/human_sample/theten.tsv", sep = '\t', header = TRUE, stringsAsFactors = FALSE)
...
## Saving as file
#write.table(txinterest[,1],"/root/tpm_genome_analysis/data/human_sample/txinterest.txt", sep = "\n", col.names = FALSE, row.names = FALSE, quote = FALSE)

### reading in quant files
gfiles<-list.files("/root/tpm_genome_analysis/data/human_sample/quant_gen/", "*sf",full.names=TRUE)
gobj<-list()
...
tots<-read.table("/root/tpm_genome_analysis/data/human_sample/tots_summ.tsv", header = FALSE, stringsAsFactors = FALSE)
...
jpeg("/root/tpm_genome_analysis/data/human_sample/fig2b.jpeg", width = 2400, height = 1000, unit = "px")
ggplot(gendat,
...
jpeg("/root/tpm_genome_analysis/data/human_sample/fig2b_relaxed.jpeg", width = 2400, height = 1200, unit = "px")
ggplot(gendat,
       aes(x = tot, y = log10(gene_tpm), color = size))+
  geom_jitter()+
#  ylim(c(-1,6))+
...
orig.file<-list.files("/root/tpm_genome_analysis/data/human_sample/", "*sf",full.names=TRUE)
...
## Plotting - Not part of manuscript anymore
jpeg("/root/tpm_genome_analysis/data/human_sample/fig.S1a.jpeg", width = 2400, height = 1000, unit = 'px')
ggplot(fdt_quant,
...
jpeg("/root/tpm_genome_analysis/data/human_sample/fig.S1b.jpeg", width = 2600, height = 1000, unit = 'px')
...
* Environment: These commands were ran in local R environment with th epackages loaded via library() as prerequisite. The only files necessary is the "theten.tsv" created in chunk 1 and the gtf annotation file from the chunk above.
* Files: Only files needed are the gtf file for human annotation (explained above), tsv file for human genes selected (created from chunk 2)
...
write.table(ten.genes.transcripts$tx,"/root/tpm_genome_analysis/data/human_sample/thetengenes_tx.tsv", sep = '\t', row.names = FALSE, col.names = FALSE, quote = FALSE)

# Reading in transcripts
df<-read.table("/root/tpm_genome_analysis/data/human_sample/thetengenes_tx.tsv", sep = '\t', header = FALSE, stringsAsFactors = FALSE)
...
exportFASTA(tseq, "/root/tpm_genome_analysis/data/human_sample/thetengenes.fa")
```

## Subsampling human genome
* First taking the sequences of the selected transcripts (from all 10 genes they represent) out of the ensembl transcriptome
* Subsampling into 99 different sizes (from 1-99% of original size based on number of transcripts included)
* Environment: These scripts were ran in LSF cluster. The software necessary is meme-5.0.5, which has been previously described above. In brief: (https://meme-suite.org/meme/meme-software/5.0.5/meme-5.0.5.tar.gz) and running ```tar -xvzf meme-5.0.5.tar.gz```

* Files: Transcript fasta created from chunk above; human genome downloaded from chunk 1.

```{bash}
mysub=/root/program/meme/libexec/meme-5.0.5/fasta-subsample
myhomo=/root/data/sequencing/genomes/human/homo_sapiens.fa
subhomo=/root/data/sequencing/genomes/human/nospike_tengenes_homo_sapiens.fa
myout=/root/size_genome/data/genomes_human_tengenes/
genes=/root/size_genome/data/genes_human/thetengenes.fa
...
* Environment: ran on commandline LSF cluster. No required software
* Files: All files necessary are generated from chunks above

```{bash}
parent=/root/size_genome/data/genomes_human_tengenes/
genes=/root/size_genome/data/genes_human/thetengenes.fa
...
* Environment: This was run as a job on LSF cluster. The only software required is salmon (described above).
* Files: All files necessary are generated from chunks above (these are the fasta files, substted and re-spiked with the target transcripts)
...
* Environment: This was run as a job on LSF cluster. The only software required is salmon (described above).
* Files: Fastq files are from sample described before and fasta files are the same as used for chunk above.
...
* Environment: Commandline interface on LSF cluster. No software required.
* Files: Files used are products from above chunk.
...
* Environment: Commandline interface on LSf cluster. No software required.
* Files: Files necessary are producst of chunk above.
...
* Environment: Commandline bash, no software necessary
* Files: Quantitation files created from chunks above.

```{bash}
parent=/root/size_genome/data/genomes_human_tengenes/summary
cd $parent
...
* Environment: Commandline bash.
* Files: The list of genes selected created above, full genome quant file (provided). 
...
## Reading in transcripts
tengenes<-read.table("/root/tpm_genome_analysis/data/human_sample/thetengenes.tsv", sep = '\t', header=FALSE)
...
write.table(thetengenes,"/root/tpm_genome_analysis/data/human_sample/thetengenes_expr.tsv", sep = '\t', row.names = FALSE, col.names = TRUE, quote = FALSE)
```

## Loading human test transcripts
* Environment: Code ran in R environment with the list of used packages loaded through library() below
* Files: The subset quantitation files from chunks above. Human annotation gtf file described above.
...
gfiles<-list.files("/root/tpm_genome_analysis/data/human_sample/quant_tengenes_target/", pattern = "*sf", full.names = TRUE)
gobj<-list()
...
## Importing full length tpm
orig<-read.table("/root/tpm_genome_analysis/data/human_sample/thetengenes_expr.tsv", sep = '\t', header = TRUE, stringsAsFactors = FALSE)
orig$Name<-gsub("\\..*","_test",orig$Name)
small<-orig[,c("Name","TPM","NumReads")]
colnames(small)<-c("gene","orig_tpm","orig_raw")
...
# Merging tx2gene to full data
df_gene<-merge(df, tx2gene, by.x = "gene", by.y = "tx") %>% dplyr::rename(tid = gene)

## Plotting
jpeg("/root/tpm_genome_analysis/data/human_sample/thetengenes_fig1a.jpeg", width = 2400, height = 1000, unit = 'px')
ggplot(df_plot,
...
jpeg("/root/tpm_genome_analysis/data/human_sample/thetengenes_fig1a_pc.jpeg", width = 2400, height = 1000, unit = 'px')
ggplot(df_plot,
       aes(x = ((size/228200)*100), fill = gene, color = gid))+
#  geom_ribbon(aes(ymin = low, ymax = hi), alpha = 0.25, colour = NA) +
  geom_line(aes(y = med),lwd=2, alpha = 0.75) +
...
## Raw plot
jpeg("/root/tpm_genome_analysis/data/human_sample/thetengenes_fig1b.jpeg", width = 2600, height = 1000, unit = 'px')
ggplot(df_raw_plot,
       aes(x = ((size/228200)*100), fill = gene, color = gid))+
#  geom_ribbon(aes(ymin = low, ymax = hi), alpha = 0.25, colour = NA) +
  geom_line(aes(y = med), lwd = 2, alpha = 0.75) +
...
# Statistical test for difference from 0
df.test<-df.full %>%
  dplyr::select(gid, size, logreltpm) %>% 
  group_by(gid,size) %>% 
  arrange(size,gid) %>% 
  filter(size != 225918) %>% 
  summarise(pval = t.test(logreltpm)$p.value)

ggplot(df.test,
       aes(x = ((size/228200)*100), y = -log10(pval), color = gid))+
  geom_line()+
  geom_hline(yintercept = 5)+
  xlab("Percent of full transcriptome")

saveRDS(df.full, "C:/Users/OZIOLE/Desktop/projects/tpm_genome_analysis/data/human_sample/theten_relquant.rds")
saveRDS(df.test, "C:/Users/OZIOLE/Desktop/projects/tpm_genome_analysis/data/human_sample/statistics_theten.rds")

## KW with Dunn's post hoc
df.test2<-df.full %>%
  dplyr::select(size, logreltpm) %>%
  group_by(size) %>%
  arrange(!size)
 

df.test2$size = factor (df.test2$size)

 

kruskal.test (logreltpm ~ size, data = df.test2)
test.res = dunn.test::dunn.test (df.test2$logreltpm, df.test2$size, method = "bh")
indeces = c (grep (pattern = " - 225918$", x = test.res$comparisons),
             grep (pattern = "^225918 - ", x = test.res$comparisons))

res = data.frame (comparison = test.res$comparisons[indeces], 
                  padj = test.res$P.adjusted[indeces])
res$size1 = unlist (lapply (res$comparison, 
                            FUN = function (x) return (as.numeric (unlist (str_split (x, " - "))[1]))
                            ))
res$size2 = unlist (lapply (res$comparison, 
                            FUN = function (x) return (as.numeric (unlist (str_split (x, " - "))[2]))
))

 

res$subgenome = apply (cbind (res$size1, res$size2), 1, 
                       FUN = function (x) return (min (x) / 2282))
 
ggplot(res, aes(x = subgenome, y = padj))+
  geom_line()+
  geom_hline(yintercept = 0.05, col = "blue") +
  scale_x_continuous (breaks = seq (0, 98, 5)) +
...
## Plotting
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_fig1a.jpeg", width = 2400, height = 1000, unit = 'px')
ggplot(df.quant,
       aes(x = ((size/228200)*100), fill = gid, color = gid))+
  geom_ribbon(aes(ymin = low, ymax = hi), alpha = 0.25, colour = NA) +
  geom_line(aes(y = med),lwd=2, alpha = 0.5) +
...
## Raw plot
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_fig1b.jpeg", width = 2400, height = 1000, unit = 'px')
ggplot(df.raw,
       aes(x = ((size/228200)*100), fill = gid, color = gid))+
  geom_ribbon(aes(ymin = low, ymax = hi), alpha = 0.25, colour = NA) +
  geom_line(aes(y = med), lwd = 2, alpha = 0.75) +
...
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_fig2a.jpeg", width = 2000, height = 1400, unit = "px")
ggplot(dat,
...
jpeg("/root/tpm_genome_analysis/data/human_sample/relaxed_tpm_vs_tots.jpeg", width = 2400, height = 1200, unit = "px")
ggplot(dat,
       aes(x = tot, y = log10(tpm), color = size))+
  geom_jitter()+
#  ylim(c(0,6))+
...
* Environment: R with packages used loaded via library().
* Files: Human gtf annotation file (described above). Table of selected genes from chunk 2. 
...
write.table(para.transcripts$tx,"/root/tpm_genome_analysis/data/human_sample/homologs.txt", sep = '\t', row.names = FALSE, col.names = FALSE, quote = FALSE)
```

## Subsampling from nospike
* Environment: Commandline bash environment. The meme-5.0.5 software is necessary (described above).
* Files: The only files necessary are the genome with ten genes removed (created above) and the homolog list, created directly above.

```{bash}
mysub=/root/program/meme/libexec/meme-5.0.5/fasta-subsample
myhomo=/root/data/sequencing/genomes/human/nospike_tengenes_homo_sapiens.fa
subhomo=/root/data/sequencing/genomes/human/nospike_nopara_tengenes_homo_sapiens.fa
myout=/root/size_genome/data/genomes_human_nopara/
genes=/root/size_genome/data/genes_human/homologs.txt
...
* Environment: Commandline bash
* Files: The fasta file of the 10 selected genes (created above).

```{bash}
parent=/root/size_genome/data/genomes_human_nopara/
genes=/root/size_genome/data/genes_human/thetengenes.fa
...
* Environment: Software necessary is salmon (described above). Submitted as a job on LSF linux cluster.
* Files: The fasta files (paralogs removed, ten genes re-spiked in) created directly above.
...
* Environment: Software necessary is salmon (described above).
* Files: Fastq files of sample (described above) and fasta files created directly above.
...
* Environment: Commandline bash.
* Files: Quantitation files from command directly above.
...
* Environment: Local R with packages needed, loaded via library() function.
* Files: Human genome annotation (described above); quantitation files created from command above.
...
### reading in quant files
gfiles<-list.files("/root/tpm_genome_analysis/data/human_sample/quant_nopara_tengenes/", "*sf",full.names=TRUE)
gobj<-list()
...
## Plotting
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_figS2a.jpeg", width = 2400, height = 1000, unit = 'px')
ggplot(df.quant,
       aes(x = ((size/228200)*100), fill = gid, color = gid))+
  geom_ribbon(aes(ymin = low, ymax = hi), alpha = 0.25, colour = NA) +
  geom_line(aes(y = med),lwd=2, alpha = 0.5) +
...
## Raw plot
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_figS2b.jpeg", width = 2400, height = 1000, unit = 'px')
ggplot(df.raw,
       aes(x = ((size/228200)*100), fill = gid, color = gid))+
  geom_ribbon(aes(ymin = low, ymax = hi), alpha = 0.25, colour = NA) +
  geom_line(aes(y = med), lwd = 2, alpha = 0.75) +
...
# Loading tots
tots<-read.table("/root/tpm_genome_analysis/data/human_sample/tots_summ_nopara.tsv", header = FALSE, stringsAsFactors = FALSE)
colnames(tots)<-c("iter","tot")
...
# Fig s3b
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_figS3b.jpeg",width = 2000, height = 1400, unit = "px")
ggplot(joy,
...
* Environment: Local R with packages necessary loaded via library() function.
* Files: 
...
# Loading Naqvi data
cyno<-fread("/root/zoomap/data/naqvi/cyno.salmon.tximport.tpm.txt")
dog<-fread("/root/zoomap/data/naqvi/dog.salmon.tximport.tpm.txt")
mouse<-fread("/root/zoomap/data/naqvi/mouse.salmon.tximport.tpm.txt")
rat<-fread("/root/zoomap/data/naqvi/rat.salmon.tximport.tpm.txt")
...
## Visualize
# jpeg("/root/tpm_genome_analysis/data/human_sample/gid_figS13.jpeg", width = 2600, height = 1000, unit = "px")
ggplot(tot.dat[subw,],
...
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_figS13.jpeg", width = 2600, height = 2000, unit = "px")
...
* Environment: Local R environment with libraries required, loaded via library()
* Files: The RDS file loaded here is provided in the repository and submission.
...
double_vioplots_with_medians_tissues = function (mydata, coi) {
  ...
  for (i in 1:n) {
    tis = mylist[i]
    the.info = data.frame (tissue = tis, 
                           x = i - 0.07, 
                           y = round (median (mydata$value[which (mydata[[coi]] == tis & 
                                                                    mydata$method == "before")]), 3), 
                           method = "before"
    )
    median.points.data = rbind (median.points.data, the.info)
  ...
  #fwrite (median.points.data, "./medPntsData.csv", sep = ",")
...
my.data = readRDS ("./Housekeeping_genes/naqvi_hkgenes.rds")
...
the.data$method = as.factor (the.data$method)

#double_vioplots_with_medians_species (mydata = the.data, coi = 'spnm')

double_vioplots_with_medians_species (mydata = the.data %>% filter (spnm != 'human'), coi = 'spnm')
...
* Environment: Commandline bash environment
* Files: none

```{bash}
parent=/root/size_genome/data/genomes_historic/fasta
cd $parent
...
```{bash}
parent_gtf=/root/size_genome/data/genomes_historic/gtf
cd $parent_gtf
...
* Environment: Using salmon (as described above) in LSF commandline environment
* Files: Files downloaded with the two chunks above
...
* Environment: Salmon (as described above) with LSF commandline bash environment
* Files: Generated with chunks directly above
...
## Historic view
* Environment: Local R environment with packages loaded from library() function.
* Files: Quantitaiton files generated with chunk directly above.
...
gtf.files<-list.files(path="/root/tpm_genome_analysis/data/human_sample/historic/gtf/", pattern = "*gz", full.names = TRUE)
...
# Creating a list of files
hfiles<-list.files("/root/tpm_genome_analysis/data/human_sample/historic/fasta/", pattern = "*sf", full.names = TRUE)
hobj<-list()
...
# Plot
jpeg("/root/tpm_genome_analysis/data/human_sample/historic/shared.tpm.jpeg",width = 1200, height = 1600, unit = "px")
ggplot(mtpm2,
...
jpeg("/root/tpm_genome_analysis/data/human_sample/historic/time.tpm.jpeg",width = 1200, height = 1600, unit = "px")
ggplot(mtpm2,
...
## Plotting
jpeg("/root/tpm_genome_analysis/data/human_sample/historic/time.ribbon.tpm.jpeg", width = 2400, height = 1000, unit = 'px')
ggplot(quant.tpm,
...
## Plotting
jpeg("/root/tpm_genome_analysis/data/human_sample/historic/time.ribbon.raw.jpeg", width = 2400, height = 1000, unit = 'px')
ggplot(quant.raw,
       aes(x = rel, color = "firebrick2", fill = "firebrick2"))+
  geom_ribbon(aes(group =1, ymin = low, ymax = hi), alpha = 0.25, colour = NA) +
  geom_line(aes(group = 1, y = med),lwd=2, alpha = 0.75) +
...
## Plotting
jpeg("/root/tpm_genome_analysis/data/human_sample/historic/time.ribbon.length.jpeg", width = 2400, height = 1000, unit = 'px')
ggplot(quant.length,
       aes(x = rel, color = "firebrick2", fill = "firebrick2"))+
  geom_ribbon(aes(group =1, ymin = low, ymax = hi), alpha = 0.25, colour = NA) +
  geom_line(aes(group = 1, y = med),lwd=2, alpha = 0.75) +
...
jpeg("/root/tpm_genome_analysis/data/human_sample/historic/time.ribbon.all.jpeg", width = 2400, height = 1000, unit = 'px')
ggplot(quant,
...
jpeg("/root/tpm_genome_analysis/data/human_sample/historic/length.v.raw.jpeg", width = 2400, height = 1000, unit = 'px')
...
* Environment: Local R environment with packages loaded via library() function
* Files: Gtf annotation files for all species examined (downloaded with chunk1)
...
# plot
jpeg("/root/tpm_genome_analysis/data/human_sample/fig3b.jpeg", width = 900, height = 900, unit = "px")
ggplot(rl,
...
# plot
jpeg("/root/tpm_genome_analysis/data/human_sample/fig4b.jpeg", width = 900, height = 900, unit = "px")
ggplot(rl,
       aes(x=species, y=native/human))+
  geom_image(aes(image=image),size=0.15)+
  ylab("Orthologs: transcripts per gene relative to human")+
...
for(i in 1:100){
  ...
  # Writing to a fasta file
  exportFASTA(tseq, paste("/root/tpm_genome_analysis/paper/giga_submission/data/tx_sub/hs_cyno", i, "fa", sep = ".", collapse = "."))
}
...
for(i in 1:100){
  ...
  # Writing to a fasta file
  exportFASTA(tseq, paste("/root/tpm_genome_analysis/paper/giga_submission/data/tx_sub/hs_dog", i, "fa", sep = ".", collapse = "."))
}
...
for(i in 1:100){
  ...
  # Writing to a fasta file
  exportFASTA(tseq, paste("/root/tpm_genome_analysis/paper/giga_submission/data/tx_sub/hs_rat", i, "fa", sep = ".", collapse = "."))
}
...
for(i in 1:100){
  ...
  # Writing to a fasta file
  exportFASTA(tseq, paste("/root/tpm_genome_analysis/paper/giga_submission/data/tx_sub/hs_mus", i, "fa", sep = ".", collapse = "."))
}
```

## Indexing 400 subsampled genomes
* Environment: Salmon (described above) ran in a LSF commandline bash env
* Files: Genomes generated from chunk above

```{bash}
parent=/root/size_genome/data/genomes_human_trpg/
cd $parent
...
# cyno
  ...
  # Writing to a fasta file
  exportFASTA(tseq, "/root/tpm_genome_analysis/paper/giga_submission/data/trpg/hs_cyno_full.fa")
...
# dog
  ...
  # Writing to a fasta file
  exportFASTA(tseq, "/root/tpm_genome_analysis/paper/giga_submission/data/trpg/hs_dog_full.fa")
...
# rat
  ...
  # Writing to a fasta file
  exportFASTA(tseq, "/root/tpm_genome_analysis/paper/giga_submission/data/trpg/hs_rat_full.fa")
...
# mus
  ...
  # Writing to a fasta file
  exportFASTA(tseq, "/root/tpm_genome_analysis/paper/giga_submission/data/trpg/hs_mus_full.fa")
...
* Environment: Salmon (described above) ran in a LSF commandline bash env
* Files: Genomes generated from chunk above and sample fastq used throughout.
...
* Files: quantitation files from chunk above
...
* Environment: Local R environment with packages loaded via library() function
* Files: Genomes generated from chunk above and gtf downloaded from chunk1
...
### reading in quant files
gfiles<-list.files("/root/tpm_genome_analysis/data/human_sample/quant_trpg/", "*full.sf",full.names=TRUE)
gobj<-list()
...
gfiles<-list.files("/root/tpm_genome_analysis/data/human_sample/quant_trpg/", "*[0-9].sf",full.names=TRUE)
gobj<-list()
...
# Plot
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_fig.S4a.jpeg", width = 1400, height = 1400, unit = 'px')
ggplot(gendat2,
...
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_fig.S4b.jpeg", width = 1400, height = 1400, unit = 'px')
ggplot(gendat2,
...
# Plotting
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_fig4a.jpeg", width = 1300, height = 2000, unit = 'px')
ggplot(gendat2,
...
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_fig4b.jpeg", width = 1300, height = 2000, unit = 'px')
ggplot(gendat2,
...
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_fig5.jpeg", width = 2000, height = 1000, unit = 'px')
ggplot(gendat2,
...
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_figS5a.jpeg", width = 2000, height = 800, unit = 'px')
ggplot(dat.sim,
       aes(x = val, color = sp))+
...
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_figS5b.jpeg", width = 2000, height = 800, unit = 'px')
ggplot(dat.len.sim,
       aes(x = val, color = sp))+
...
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_figS7a.jpeg", width = 2600, height = 1000, unit = 'px')
...
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_figS7b.jpeg", width = 2600, height = 1000, unit = 'px')
...
* Environment: Local R environment with packages loaded via library() function
* Files: Genomes generated from chunks above
...
### reading in quant files
gfiles<-list.files("/root/tpm_genome_analysis/data/human_sample/quant_trpg/", "*cyno_full.sf",full.names=TRUE)
gobj<-list()
...
### reading in quant files
gfiles<-list.files("/root/tpm_genome_analysis/data/human_sample/quant_trpg_short/", "*cyno",full.names=TRUE)
...
# Plotting
jpeg("/root/tpm_genome_analysis/data/human_sample/gid_figS9.jpeg", width = 2000, height = 1000, unit = 'px')
ggplot(gendat2,
